...
{
    ClipOut r{};
    r.eps_ca = std::clamp(eps_ca, 0.0, eps_max);
    ...
    r.kappa_eff = std::clamp(kappa, 0.0, r.kappa_max);
    return r;
}
...
EpsSolveResult SectionCal::solve_eps_ca_for_kappa(
    ...
    if (ra == 0.0) {
        ...
        if ((ra > 0.0) == (rb > 0.0)) {
            ...
            for (int i = 1; i <= Nscan; ++i) {
                ...
                if (!std::isfinite(rx) || !std::isfinite(r_prev)) {
                    x_prev = x;
                    r_prev = rx;
                    continue;
                }
    ...
    auto [res, mom] = this->forceresidual_moment(out.eps_ca, out.kappa_eff);
    out.residual = res;
    out.moment = mom;

    out.success = std::isfinite(out.residual) && std::isfinite(out.moment);
    return out;
}
